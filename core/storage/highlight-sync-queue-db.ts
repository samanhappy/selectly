/**
 * Highlight Sync Queue Database
 * Stores pending sync operations (create, update, delete) for highlight items
 */

import Dexie from 'dexie';
import type { Table } from 'dexie';

export type HighlightSyncOperation = 'create' | 'update' | 'delete';

export interface HighlightSyncQueueItem {
  id?: number;
  itemId: string;
  operation: HighlightSyncOperation;
  timestamp: number;
  retryCount: number;
  lastError?: string;
}

class HighlightSyncQueueDB extends Dexie {
  queue!: Table<HighlightSyncQueueItem, number>;

  private static instance: HighlightSyncQueueDB;

  static getInstance() {
    if (!HighlightSyncQueueDB.instance) {
      HighlightSyncQueueDB.instance = new HighlightSyncQueueDB();
    }
    return HighlightSyncQueueDB.instance;
  }

  private constructor() {
    super('selectly-highlight-sync-queue-db');
  ...
  async enqueue(itemId: string, operation: HighlightSyncOperation) {
...
export const highlightSyncQueueDB = HighlightSyncQueueDB.getInstance();
